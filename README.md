...
CLI 读取 `.claude-plugin/marketplace.json`，把 skill 软链到 `~/.claude/skills/`。安装时会提示选择要安装的分组（Engineering / Research / 全部）。安装后，在 agent 里直接用 `/dispatch`、`/git-commit`、`/to-prd`、`/handoff`、`/grill-with-docs` 等命令。
...
**解法**：[`/git-commit`](./skills/engineering/git-commit/SKILL.md) 分析当前会话改动，给出精准的提交建议（应提交哪些文件、改动摘要、拟定 message），避免误提交不相关文件。

### 3. 计划写完就丢，执行靠人肉

写完计划或 handoff 文件后，得手动拆任务、逐个分配给 agent、盯进度、收结果。计划和执行之间全是手工活。

**解法**：[`/dispatch`](./skills/engineering/dispatch/SKILL.md) 读取计划或 GitHub issue，自动按依赖关系拆解任务并并行执行，失败时汇报、成功后推进，最终跑测试验证。
...
讨论计划时同一概念多种叫法、边界不清，重要架构决定口头说说就过了。
...
**解法**：基于《研究是一门艺术》，提供 8 个按研究流程组织的 skills：从研究规划、问题界定、资料收集与分析，到论证构建、报告撰写、视觉呈现，再到研究伦理。
...
9. **PDF 解析**：用 [`/pdf-with-mineru`](./skills/engineering/pdf-with-mineru/SKILL.md) 把 PDF 转 markdown。
10. **架构审计**：用 [`/improve-codebase-architecture`](./skills/engineering/improve-codebase-architecture/SKILL.md) 生成报告。
11. **调试与排障**：用 [`/diagnosing-bugs`](./skills/engineering/diagnosing-bugs/SKILL.md) 诊断 bug，[`/resolving-merge-conflicts`](./skills/engineering/resolving-merge-conflicts/SKILL.md) 处理冲突。
12. **系统化研究**：按研究流程使用 skills：先 [`/research-planning`](./skills/research/research-planning/SKILL.md) 规划；再 [`/problem-identification`](./skills/research/problem-identification/SKILL.md) 确定题目；然后 [`/source-collection`](./skills/research/source-collection/SKILL.md) 搜集资料；接着 [`/source-analysis`](./skills/research/source-analysis/SKILL.md) 分析；之后 [`/argument-construction`](./skills/research/argument-construction/SKILL.md) 构建论证；再 [`/report-writing`](./skills/research/report-writing/SKILL.md) 撰写报告；同时 [`/visual-presentation`](./skills/research/visual-presentation/SKILL.md) 完善呈现；最后 [`/research-ethics`](./skills/research/research-ethics/SKILL.md) 确保学术诚信。
