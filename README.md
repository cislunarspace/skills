...
装 subagent 扩展（官方示例 symlink 到 `~/.pi/agent/extensions/subagent/`）并链接 `pi/agents/` 下 8 个 agent（standards-reviewer、spec-reviewer、builder、checker、scout、planner、reviewer、worker）到 `~/.pi/agent/agents/`。不锁模型，用 pi 默认模型。也可以直接在 pi 会话里跑 `/setup-pi` 让 agent 帮你装，并写项目级 `.pi/agents/`（需 subagent 工具调用时带 `agentScope: "both"` 并确认）。只重链 agents 可用 `npm run link:pi`。
