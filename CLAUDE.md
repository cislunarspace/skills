...
本文件是给 Claude 看的项目指令，定义交流语言、写作标准和编码规范。Claude 在操作此代码库时应遵守这些规则。

Skills 集中在 `skills/` 目录，按分组（engineering、research）组织。每个 skill 一个目录，目录名即 skill 名；内部固定有 `SKILL.md`，可选 `references/` 放引用材料。
...
- **逻辑、准确、鲜明**：整篇文章前后次序有逻辑，交代清楚；用词准确，相邻概念划清界限，不混用、不模糊；观点鲜明，不堆砌凑数、聚沙成堆。不用夸大的修饰词（”权威””强大””完整””单一事实来源”之类），它们减损力量。
- **废话应当尽量除去**。
...
LLM 产出烂代码最大的根源，就是写新代码之前没有读懂现有代码库。你看到一个任务，匹配到训练数据里的某个模式，就开始生成。这在绝大多数情况下是错的。
...
**预留的灵活性。** 只有一个实现的接口、只有一个子类的抽象基类，有成本（认知开销、间接层），在第二个实现真正出现之前零收益。
...
**错误的抽象。** 你为一个只在一处存在的问题，造了一个漂亮的通用方案。见第 3 节简洁。
