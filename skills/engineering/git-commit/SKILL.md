...
2. 运行 `git diff --name-only`，识别不在该列表中的脏文件；它们属于会话外改动。
3. 运行 `git diff --stat` 和 `git diff -- <会话文件>`，确认每项改动都应提交。
4. 读取 `references/git-workflow.md`（存在时），据此起草 commit message；没有该文件则用常规格式。
5. 从对话上下文识别关联 issue（`#N` 或 issue 链接）。

会话文件为空、存在冲突，或 diff 与本会话目标不符时，停止并说明原因。

### 2. 确认并提交

1. 展示会话文件、会话外脏文件、改动摘要和拟议 commit message。
