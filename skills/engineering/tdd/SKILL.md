...
探索代码库时，读 `CONTEXT.md`（如果存在），让测试名和接口词汇匹配项目的领域语言；尊重你要触碰区域的 ADR。
...
**接缝**（seam）是你测试的公开边界：你能观察行为、又不把手伸进内部的接口。测试落在接缝上，永远不直接对内部下手。

**只在预先商定的接缝上测试。** 写任何测试之前，把要测的接缝列出来、跟用户确认。没确认过的接缝不写测试。你没法什么都测——预先约定接缝，才能让测试精力落在关键路径和复杂逻辑上，而不是散到每个边界情况。

问："公开接口是什么？该测哪些接缝？"

当那个接口的形状本身存疑时——模块有多深、接缝归在哪、接口该暴露什么——用 `/codebase-design` skill 获取词汇。它是 模块、接口、深度、接缝、适配器、杠杆、局部性 这些术语的共享来源，是一份供查阅的参照，不是一次要跑的会话。
