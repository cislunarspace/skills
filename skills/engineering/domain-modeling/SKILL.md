...
description: 构建和打磨项目的领域模型。当用户想确定领域术语或统一语言、记录一项架构决策，或别的 skill 需要维护领域模型时使用。
---

# 领域建模

在设计过程中主动构建和打磨项目的领域模型。这是一项*主动的*纪律——挑战术语、构造边界场景、在术语和决策刚结晶的那一刻就把它们写下来。（仅仅是*读* `CONTEXT.md` 找词汇不算这个 skill——那是任何 skill 都能做的一行习惯。这个 skill 用在你正在改变模型、而不只是消费模型的时候。）
