...
test('setup references 的 4 个种子文件都存在', () => {
  for (const name of SEED_FILES) {
    const filePath = path.join(SETUP_REFERENCES, name);
...
test('setup references 种子文件无 U+00A0 与 CR', () => {
  for (const name of SEED_FILES) {
    const buf = fs.readFileSync(path.join(SETUP_REFERENCES, name));
...
test('builder/checker 种子保留 agent 定义关键字段', () => {
  const builder = fs.readFileSync(path.join(SETUP_REFERENCES, 'builder.md'), 'utf8');
  ...
  const checker = fs.readFileSync(path.join(SETUP_REFERENCES, 'checker.md'), 'utf8');
...
test('loop-go-command.md 保留命令关键字段', () => {
  const content = fs.readFileSync(path.join(SETUP_REFERENCES, 'loop-go-command.md'), 'utf8');
