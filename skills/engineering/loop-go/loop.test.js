...
// 2. setup 技能的 4 个种子文件必须保持干净（无 U+00A0、无 CR），
...
const SETUP_CLAUDE_REFERENCES = path.resolve(__dirname, '..', 'setup-claude-code', 'references');
const SETUP_PI_REFERENCES = path.resolve(__dirname, '..', 'setup-pi', 'references');

const CLAUDE_SEED_FILES = [
  'builder.md',
  'checker.md',
  'loop-go-command.md',
  'loop-stop-rules.md',
...
test('setup references 种子文件都存在（setup-claude-code + setup-pi）', () => {
  for (const name of CLAUDE_SEED_FILES) {
    const filePath = path.join(SETUP_CLAUDE_REFERENCES, name);
    assert.ok(fs.existsSync(filePath), `${name} 应存在于 setup-claude-code/references`);
  }
  for (const name of PI_SEED_FILES) {
    const filePath = path.join(SETUP_PI_REFERENCES, name);
    assert.ok(fs.existsSync(filePath), `${name} 应存在于 setup-pi/references`);
  }
});

test('setup references 种子文件无 U+00A0 与 CR', () => {
  for (const name of CLAUDE_SEED_FILES) {
    const buf = fs.readFileSync(path.join(SETUP_CLAUDE_REFERENCES, name));
    assert.ok(!buf.includes(0x0d), `${name} 不应含 CR (0x0d)`);
    assert.ok(!buf.toString('utf8').includes(' '), `${name} 不应含 U+00A0`);
  }
  for (const name of PI_SEED_FILES) {
    const buf = fs.readFileSync(path.join(SETUP_PI_REFERENCES, name));
    assert.ok(!buf.includes(0x0d), `${name} 不应含 CR (0x0d)`);
    assert.ok(!buf.toString('utf8').includes(' '), `${name} 不应含 U+00A0`);
  }
});
...
test('builder/checker 种子保留 agent 定义关键字段', () => {
  const builder = fs.readFileSync(path.join(SETUP_CLAUDE_REFERENCES, 'builder.md'), 'utf8');
  ...
  const checker = fs.readFileSync(path.join(SETUP_CLAUDE_REFERENCES, 'checker.md'), 'utf8');
  ...
  const builderPi = fs.readFileSync(path.join(SETUP_PI_REFERENCES, 'builder-pi.md'), 'utf8');
  assert.match(builderPi, /^name:\s*builder$/m, 'builder-pi.md 应含 name: builder');
  assert.match(builderPi, /^tools:\s*read, write, edit, grep, find, ls, bash$/m, 'builder-pi.md 应含 pi 小写工具');

  const checkerPi = fs.readFileSync(path.join(SETUP_PI_REFERENCES, 'checker-pi.md'), 'utf8');
  assert.match(checkerPi, /^name:\s*checker$/m, 'checker-pi.md 应含 name: checker');
  assert.match(checkerPi, /^tools:\s*read, grep, find, ls, bash$/m, 'checker-pi.md tools 应无写工具');
  assert.ok(!checkerPi.includes('write') && !checkerPi.includes('edit'), 'checker-pi 不应有写工具');
});

test('loop-go-command.md 保留命令关键字段', () => {
  const content = fs.readFileSync(path.join(SETUP_CLAUDE_REFERENCES, 'loop-go-command.md'), 'utf8');
...
test('loop-stop-rules.md 含停止条件与红线', () => {
  const content = fs.readFileSync(path.join(SETUP_CLAUDE_REFERENCES, 'loop-stop-rules.md'), 'utf8');
