...
# Setup ouuyangjiahong's Skills

为当前仓库搭建工程技能所需的配置：

- **Issue tracker**：issue 存在哪里（默认 GitHub；也支持本地 markdown）
- **分诊标签**：五个标准分诊角色对应的标签字符串
...
总结哪些已存在、哪些缺失。然后**一次一个**地引导用户做三个决定，展示一节、拿到答案，再进入下一节。

假设用户不了解这些术语。每节开头给一段简短解释（是什么、为什么技能需要它、不同选择会怎样）。然后给出选项和默认值。

**A 节（Issue tracker）。**

> 解释："Issue tracker"是本仓库 issue 存放的地方。`to-issues`、`to-prd`、`qa` 等技能要读写它，它们需要知道该调 `gh issue create`、在 `.scratch/` 下写 markdown 文件，还是按你描述的其他流程走。选你实际用来跟踪工作的地方。
...
> 解释：开源仓库常以外界 PR 形式收到功能请求，PR 就是带代码的 issue。开启后，`/triage` 会把外部 PR 拉入同一个队列，用与 issue 相同的标签和状态处理（协作者进行中的 PR 不动）。如果 PR 不是你的请求渠道，关闭即可。
