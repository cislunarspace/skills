...
不要输出待写入文件的完整正文，避免消耗大量输出 token。只告知用户：

- 将写入或更新哪些文件；
- 每个文件使用的种子模板路径（模板位于本技能的 `references/` 目录，完整映射见下方）；
- 对已有文件会保留、替换或追加哪些结构。

告诉用户可以直接查看这些模板和仓库中的现有文件；用户确认后直接进入写入，不再重复输出正文。
...
种子模板（位于本技能 `references/` 下，确认阶段只告知路径，不输出正文）：
