...
如果你有一个**紧**的反馈回路——一个在**这个** bug 上会变红的回路——你就一定能找到根因；二分、验证假设、加打点，全都只是消费这个信号。没有这样的回路，盯着代码看到天亮也没用。
...
停下来，明说。列出你已经试过的。问用户要：(a) 能复现它的那个环境的访问权限，(b) 一份抓到的产物（HAR 文件、日志转储、core dump、带时间戳的录屏），或 (c) 加临时生产打点的许可。没有回路时，不要进入后续阶段。
...
**回归测试是修复的一部分，不是可选项。** 没有回归测试的修复不算完成——修完 bug 后必须有一个测试：在修复前会失败（证明它能捕获这个 bug），在修复后会通过。
