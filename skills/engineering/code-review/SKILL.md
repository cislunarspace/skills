...
2. 用户作为参数传入的路径。
3. 本会话中用户的原始请求——这批提交出自当前会话时，用户当初的指令就是规格；无 issue 的改动多属此类。
...
两个子代理返回后，先把返回值还原成两份纯文本报告再进第 5 步。子代理的返回形态因 harness 而异（纯文本 / 结构化对象 / 落盘文件）：结构化对象先看清结构、取承载文本的字段；落盘文件直接读。提取失败时诊断返回结构本身——子代理已跑完，重跑是最后手段。
