...
1. **空操作测试**：逐句问——"删掉这句后 agent 行为有区别吗？" 没区别就删。删整句，不修字。
2. **六种失败模式检查**：过早结束、重复、沉积、膨胀、空操作、否定。详见 `docs/skill-writing.md`。
3. **信息层级检查**：参考内容是否放对了位置？每个分支都需要的 inline，只有部分需要的放在外部文件。
...
- 新 skill 落盘后：把它加进 `.claude-plugin/marketplace.json` 对应 plugin 的 `skills` 数组
- 新 skill 想立刻试一下：重启 Claude Code 会话让 marketplace 重新加载
