...
脚本包揽全部：归一化源文件行尾、从 STANDARDS.md 重建 `templates/`、对每个目标文件做节级替换或追加、确保 H1 标题匹配文件名（`# CLAUDE.md` / `# AGENTS.md`）、LF 归一化、验证三节与源逐字一致。退出码 0 即成功，非 0 会打印原因。
...
- `templates/` 是派生产物——脚本每次运行从 STANDARDS.md 重建，无需手动维护。
