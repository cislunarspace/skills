# 注入规范原文（sync-writing-standards）

<!-- 以下是注入目标 AGENTS.md 的规范原文。每个 ## 节是一个注入单元：
     目标 AGENTS.md 已有同名 ## 节则整节替换，没有则追加到末尾。
     本 h1 标题与本注释不属于注入内容。文件中 ## 之间的分隔线同样不单独注入。 -->
...
- **注意词语的逻辑界限**：相邻概念要划清，不混用、不模糊。概念不是事物的现象，不是事物的各个片面，不是它们的外部联系，而是抓住了事物的本质，事物的全体，事物的内部联系。循此推进，使用判断和推理的方法，就可产生出合乎论理的结论来。
- **废话应当尽量除去**。
