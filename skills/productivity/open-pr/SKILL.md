---
name: open-pr
description: 把本地分支推到远端并完成 PR 合并的完整生命周期——push 分支、开 PR、code review、合并、清理本地分支。当用户说"开 PR"、"提 PR"、"open PR"、"提交分支到 master"，或在一个分支上完成工作后希望端到端落地时使用。
...
**出发前先判断**：本轮会话是否已经对同一分支跑过 `/code-review`？

- 若还没跑过 review → 进 step 4a，重新执行 code review。
- 若已经跑过 review（比如用户先 `/code-review` 审完修完，再 `/open-pr` 合并）→ **不要重复执行**。直接把之前 review 的结论搬过来，跳到 step 5。

#### 4a. 执行 Code Review（仅当本轮未 review 时）
...
如果 review 是从本轮已跑过的 `/code-review` 复用的，则在报告前加一行：

```
ℹ️ 本轮会话已跑过 code review，直接复用之前的结论。
```
...
| 本轮会话已跑过 code-review | step 4 检测到直接复用，不重复执行。打印 `ℹ️` 提示，跳到 step 5 |
