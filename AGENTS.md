...
新增 skill：在 `skills/<group>/<name>/` 下放 `SKILL.md`（写作规范见 `docs/skill-writing.md`，骨架模板在 `docs/templates/SKILL.md`），在 `.claude-plugin/plugin.json` 的 `skills` 数组里加一行，重跑 `npx skills add cislunarspace/skills`。
